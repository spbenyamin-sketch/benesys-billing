...
export default function SalesB2C() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (paymentMode === "CASH" && amountGiven === 0) {
        throw new Error("Please enter amount given");
      }
    ...
    onSuccess: (data: any) => {
      ...
      toast({
        title: "Success",
        description: "B2C Retail Invoice saved successfully",
      });
          ...
          <CardContent className="pt-4">
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-bold">{totals.totalQty.toFixed(3)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-muted-foreground">Sale Value</span>
                <span className="font-bold">₹{totals.saleValue.toFixed(2)}</span>
              </div>
              {totals.discountTotal > 0 && (
                <div className="flex justify-between text-green-600">
                  <span className="text-xs">Discount</span>
                  <span className="font-bold">-₹{totals.discountTotal.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-xs text-muted-foreground">{gstType === 0 ? "CGST+SGST" : "IGST"}</span>
                <span className="font-bold">₹{totals.taxValue.toFixed(2)}</span>
              </div>
              {totals.roundOff !== 0 && (
